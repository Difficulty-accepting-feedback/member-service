...
public class MemberNameServiceImpl implements MemberNameService {
    ...
    public String findMemberNameById(Long memberId) {
        log.info("[MEMBER][SEARCH] 회원 이름 조회 요청 시작 - memberId={}", memberId);

        Member findMember = memberRepository.findById(memberId).orElseThrow(
                () -> new ServiceException(ErrorCode.MEMBER_NOT_FOUND));

        log.info("[MEMBER][SEARCH] 회원 이름 조회 요청 완료 - memberId={}, nickname={}", memberId, findMember.getMemberProfile().getNickname());
    ...
    public MemberInfo findMemberInfoById(Long memberId) {
        log.info("[MEMBER][SEARCH] 회원 정보 조회 요청 시작 - memberId={}", memberId);

        Member findMember = memberRepository.findById(memberId).orElseThrow(
                () -> new ServiceException(ErrorCode.MEMBER_NOT_FOUND));

        log.info("[MEMBER][SEARCH] 회원 정보 조회 요청 완료 - nickname={}, score={}", findMember.getMemberProfile().getNickname(), findMember.getScore());
